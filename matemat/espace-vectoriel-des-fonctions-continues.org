...
Si $\corps$ est un corps, on vérifie que $\continue(A,\corps)$ est
un espace vectoriel sur $\corps$. En effet, la fonction nulle $0$ est
clairement continue. De plus, si $\alpha,\beta \in \corps$ et si $f,g
\in \continue(A,\corps)$, on a :

\begin{align*}
\lim_{x \to a} (\alpha \cdot f(x) + \beta \cdot g(x))
&= \alpha \cdot \lim_{x \to a} f(x) + \beta \cdot \lim_{x \to a} g(x) \\
