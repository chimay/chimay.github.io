...
* Bilinéarité
:properties:
:custom_id: heading:bilinearite
:end:

Soit $\varphi,\psi \in E^\dual$, $u,v \in E$ et $\alpha,\beta \in
\corps$. Comme $\varphi$ est linéaire, on a :
...
* Application adjointe ou duale
:properties:
:custom_id: heading:application_adjointe_ou_duale
:end:

Soit les espaces vectoriels $E$ et $F$ sur $\corps$ et une fonction $A :
E \mapsto F$. Si on peut trouver une unique
fonction $A^\dual : F^\dual \mapsto E^\dual$ telle que :

$$\forme{ A^\dual(\varphi) }{u} = \forme{\varphi}{ A(u) }$$

pour tout $u \in E$ et $\varphi \in F^\dual$, on dit que $A^\dual$
est l’application adjointe ou l’application duale de $A$ au sens
