...
progression, d’un point de la droite vers un autre. La progression ne
se limite toutefois pas au segment compris entre ces deux points : on
peut partir de n’importe quel point de la droite, et progresser aussi
loin qu’on le souhaite. Dans la suite d’ouvrages mathématiques de
ce site, je note :
...
d’une extrémité du segment vers l’autre. Dans la suite d’ouvrages
mathématiques de ce site, je note :
...
On peut préciser le centre et le rayon directement dans la notation :
...
Un arc de cercle peut être orienté positivement ou négativement.

Par convention, un arc de cercle positif $\Gamma^+$ tourne dans le sens
inverse des aiguilles d’une monte :

#+attr_html: :width 25%
#+attr_latex: :width 0.7\linewidth
[[file:tikz/arc-de-cercle-oriente-positif.svg]]

tandis qu’un arc de cercle négatif $\Gamma^-$ tourne dans le sens
des aiguilles d’une monte :

#+attr_html: :width 25%
#+attr_latex: :width 0.7\linewidth
[[file:tikz/arc-de-cercle-oriente-negatif.svg]]
