...
à la longueur de la ligne droite qui sépare ces deux points. Or,
cette longueur est égale à la longueur du segment $[A,B]$. On a donc :
...
Le schéma ci-dessous représente trois points alignés $A$, $B$ et $C$ :

#+attr_html: :width 30%
#+attr_latex: :width 0.7\linewidth
[[file:tikz/points-alignes.svg]]

L’alignement nous donne une propriété supplémentaire sur les
distances. Dans ce cas particulier, on a :
...
Une tangente $t$ au cercle $\mathscr{C}$ passant par un point $A$ est une
droite qui intersecte $\mathscr{C}$ seulement en $A$ :

$$ \mathscr{C} \cap t = P $$

Le schéma ci-dessous représenté la tangente $t = (AB)$ au cercle $\mathscr{C}$ :
