...
Soit les fonctions $f,S,I : \setR \mapsto \setR$ majorées, minorées
et vérifiant $I \le f \le S$. Supposons que :
...
$$\lim_{x \to +\infty} f(x) = \lim_{x \to +\infty} I(x) = \lim_{x \to +\infty} S(x)$$
