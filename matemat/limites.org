...
Plaçons nous dans $A \subseteq D$. Nous nous intéressons au cas où
$f(x)$ se rapproche de plus en plus d'un certain $L \in F$ lorsque $x \in
A$ se rapproche suffisamment d'un certain $a \in E$. Pour toute précision
$\epsilon \in \corps$, $\epsilon \strictsuperieur 0$ aussi petite
que l'on veut, on doit alors pouvoir trouver un niveau de proximité
$\delta(\epsilon) \in \corps$, $\delta(\epsilon) \strictsuperieur 0$
tel que :
