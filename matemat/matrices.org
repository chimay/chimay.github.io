...
pour tout $i\in\{1,2,...,m\}$ et $j\in\{1,2,...,n\}$.

Cette constatation nous pousse à représenter $\mathcal{A}$ par le
tableau de $m$ lignes et $m$ colonnes :
...
Un tableau $A$ de cette forme est appelé matrice de taille $(m,n)$.
...
où les $a_{ij}$ appartiennent à un corps $\corps$. On a également
des versions simplifiées de cette notation :
...
$n$ colonnes. On dit d'une matrice $A \in \matrice(\corps,m,n)$ qu'elle
est de taille $(m,n)$.
