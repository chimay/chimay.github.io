...
Transposer un vecteur colonne consiste à le transformer en vecteur
ligne, et vice versa.

Pour généraliser ce concept à une matrice $A \in \matrice(\corps,m,n)$
définie par :
...
et en transposer chaque colonne pour obtenir des lignes que nous pouvons
alors superposer :
...
et en transposer chaque ligne pour obtenir des colonnes que nous pouvons
alors juxtaposer :

$$ A^T = [l_1^T \ \ l_2^T \ \ \ldots \ \ l_m^T] $$

Ces deux méthodes produisent le même résultat, à savoir une matrice
transposée $A^T$ définie par :

$$ A^T = (a_{ji})_{i,j} $$

Il suffit donc de permuter les deux indices des composantes pour obtenir
