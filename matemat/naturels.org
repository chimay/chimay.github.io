...
Augmenter le nombre de colonnes de $1$ augmente le nombre de cases total
du nombre de lignes $m$ :
...
Les relations ci-dessus peuvent alors se réécrire :
...
Il va de soi qu’un tableau contenant une seule ligne et $n$ colonnes contient au total
$n$ cases :

$$ 1 \cdot n = n $$

et qu’un tableau contenant une seule colonne et $m$ lignes contient
au total $m$ cases :
...
Un tableau de $m$ lignes et $n$ colonnes contient autant de cases qu’un
tableau de $n$ lignes et $m$ colonnes. On a donc :
...
En plaçant côte à côte un tableau $T$ de $m$ lignes et $n$ colonnes
avec un tableau $U$ de $m$ lignes et $p$ colonnes, on obtient un nombre
de cases total :

$$ m \cdot n + m \cdot p $$

Mais le tableau formé en associant $T$ et $U$ compte $m$ lignes et $n + p$ colonnes,
...
Il va de soi qu’un tableau ne contenant aucune ligne contient zéro
case :
