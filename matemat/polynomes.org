...
La valeur $a$ est donc une racine de $p$. Si on utilise la méthode de
Horner pour diviser $p$ par $x - a$, on aura un reste nul et :
...
q_{n-1} &= 1 \\
q_{n-2} &= 0 + a \cdot 1 = a \\
q_{n-3} &= 0 + a \cdot a = a^2 \\
q_{n-4} &= 0 + a \cdot a^2 = a^3 \\
\ldots & \ldots \\
q_{n - 1 - i} &= a^i \\
\ldots & \ldots \\
q_0 &= a^{n - 1} \\
...
Le polynôme $q$ peut donc se réécrire :
