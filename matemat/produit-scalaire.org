...
pour tout $u,v \in E$. Ce produit hérite bien entendu la
[[file:formes-lineaires.org::#heading:bilinearite][bilinéarité]]
de la forme associée :

$$\scalaire{w}{\alpha \cdot u + \beta \cdot v}  = \alpha \cdot
\scalaire{w}{u} + \beta \cdot \scalaire{w}{v}$$ $$\scalaire{\alpha
\cdot u + \beta \cdot v}{w}  = \alpha \cdot \scalaire{u}{w} + \beta
\cdot \scalaire{v}{w}$$

pour tout $\alpha,\beta \in \corps$ et pour tout $u,v \in E$.

* Produit scalaire réel

Considérons le cas particulier où $\corps = \setR$. Un produit scalaire
réel est un produit bilinéaire $\scalaire{}{} : E \times E \mapsto
\setR$ :

$$\scalaire{w}{\alpha \cdot u + \beta \cdot v}  = \alpha \cdot \scalaire{w}{u} + \beta \cdot \scalaire{w}{v}$$

$$\scalaire{\alpha \cdot u + \beta \cdot v}{w}  = \alpha \cdot \scalaire{u}{w} + \beta \cdot \scalaire{v}{w}$$

pour tout $\alpha,\beta \in \setR$ et $u,v,w \in E$. Nous imposons en plus
que la valeur de $\scalaire{u}{u}$ en chaque $u \in E$ puisse représenter
le carré de la norme de $u$. Nous imposons donc la positivité :

$$\scalaire{u}{u} \ge 0$$

La norme étant un cas particulier de distance, on impose également
que le seul élément $u \in E$ vérifiant :
...
Enfin, le produit scalaire est symétrique, c’est-à-dire qu’on peut
interchanger n'importe quels $u,v \in E$ sans modifier le résultat :

$$\scalaire{u}{v} = \scalaire{v}{u}$$

** Notation

Dans le cas de vecteurs géométriques, on note aussi le produit scalaire
sous forme de point :
...
Examinons à présent le cas $\corps = \setC$. On demande qu'un produit
scalaire $\scalaire{}{} : E \times E \mapsto \setC$ soit strictement
défini positif. Pour cela, les valeurs de $\scalaire{u}{u}$ doivent
être réelles et positives :

$$ \scalaire{u}{u} \in \setR
\qquad \qquad \qquad
\scalaire{u}{u} \ge 0 $$

pour tout $u \in E$. Ensuite, il faut également que le seul élément
$u \in E$ vérifiant :
...
où la barre supérieure désigne comme d'habitude le complexe
conjugué. Cette constatation nous mène à une variante de la
symétrie. On impose :

$$\scalaire{u}{v} = \conjaccent{\scalaire{v}{u}}$$

pour tout $u,v \in E$. On dit que le produit scalaire complexe est
hermitien.

La linéarité à droite s'exprime simplement par :

$$ \scalaire{w}{\alpha \cdot u + \beta \cdot v} =
\alpha \cdot \scalaire{w}{u} + \beta \cdot \scalaire{w}{v} $$

pour tout $\alpha,\beta \in \setC$ et pour tout $u,v,w \in E$. On déduit
de cette linéarité et du caractère hermitien du produit scalaire
complexe que :

\begin{align*}
\scalaire{\alpha \cdot u + \beta \cdot v}{w}
&= \conjaccent{\scalaire{w}{\alpha \cdot u + \beta \cdot v}} \\
...
$$ \scalaire{\alpha \cdot u + \beta \cdot v}{w} =
\conjaccent{\alpha} \cdot \scalaire{u}{w} + \conjaccent{\beta} \cdot \scalaire{v}{w}$$

On dit que le produit scalaire complexe est :

- antilinéaire ou semi-linéaire à gauche
- une forme sesquilinéaire (linéaire et demie)
...
On peut voir que si une suite de vecteurs $e_i$ est orthogonale (ils
ne forment pas forcément une base), ils sont toujours linéairement
...
On peut voir que si une suite de vecteurs $e_i$ est orthonormée (ils
ne forment pas forcément une base), ils sont toujours linéairement
indépendant. En effet si les scalaires $a_i$, sont tels que :
...
On vérifie que la base canonique $(\canonique_1,...,\canonique_n)$ est orthonormée pour
ce produit scalaire :

$$\canonique_i^T \cdot \canonique_j = \indicatrice_{ij}$$
...
pour certains $x_i,y_i \in \corps$. Or, nous avons vu que :
...
* Bases de vecteurs colonnes

Un cas particulier important survient lorsque les vecteurs sont des vecteurs colonnes. Soit une suite de vecteurs linéairement indépendants $u_1,u_2,...,u_m \in \matrice(\corps,n,1)$. Soit des $x_i,y_i \in \corps$ et les vecteurs :
