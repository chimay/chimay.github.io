...
*Remarque* : la notation $(a,b)$ qui représente un nombre rationnel
comme un couple formé par les nombres entiers $a$ et $b$ est uniquement
...
pour tout $m,n,i,j \in \setN$ tels que :
...
Soit $a,b,c,d \in \setZ$ avec $b,d \ne 0$. Nous allons étendre l'ordre
sur l'ensemble des rationnels en imposant que la multiplication par un
nombre strictement positif conserve l'ordre et que la multiplication
par un nombre strictement négatif l'inverse. Soit l'inégalité :
...
Soit un rationnel $x$ positif, qui s’écrit sous la forme canonique :

$$ x = \frac{a}{b} \ge 0 = \frac{0}{1} $$

Si $b$ est positif, on a par définition :
...
Par contre, si $b$ est négatif, on a par définition :
...
Pour qu’un nombre rationnel soit positif, il faut que le numérateur et
le dénominateur soient tous les deux positifs ou tous les deux négatifs.
Ils doivent donc être de même signe.

** Négatifs

On définit les ensembles des rationnels négatifs par :

$$\setQ^- = \{ x \in \setQ : x \le 0 \}$$

Soit un rationnel $x$ négatif, qui s’écrit sous la forme canonique :
...
Pour qu’un nombre rationnel soit négatif, il faut que le numérateur
et le dénominateur soient de signes contraires.
...
/Remarque/ : On utilise parfois le point au lieu de la virgule :

$$ i_m ... i_0. j_1 ... j_n = i_m ... i_0, j_1 ... j_n $$

Exemple :

$$ 123.456 = 123,456 $$

Attention à ne pas confondre le point décimal avec le symbole $\cdot$
de multiplication :
