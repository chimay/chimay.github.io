...
* Propriétés des sommes convergentes

** Additivité

Si les sommes des $a_i$ et des $b_i$ convergent, alors les $a_i + b_i$
aussi et on a :

$$\sum_{i=0}^{+\infty} ( a_i + b_i ) = \sum_{i=0}^{+\infty} a_i + \sum_{i=0}^{+\infty} b_i$$

Pareil pour les $a_i - b_i$ :

$$\sum_{i=0}^{+\infty} ( a_i - b_i ) = \sum_{i=0}^{+\infty} a_i - \sum_{i=0}^{+\infty} b_i$$

** Décalage

Si la somme des $a_i$ converge, alors la somme des $b_i$ définis par :
...
$$\sum_{i=0}^{+\infty} b_i = \sum_{i=0}^{+\infty} a_i$$

* Sommes de Cesaro

Voir [[https://www.youtube.com/watch?v=YuIIjLr6vUA][la vidéo de Mathologer]]
...
Si la suite des $M_n$ converge, elle respecte les propriétés
d’additivité et de décalage des sommes convergentes classiques.

Si la suite des $S_n$ converge, alors la suite des $M_n$ aussi
et :
