...
% segments, sides, lines labels %{{{1
	...
	\node[below right] at ($ (O)!0.5!(P_8) $) {$r$};

	%\node[above] at ($ (P_2)!0.35!(P_8) $) {$d$};

% segments, sides, lines marks %{{{1

	\tkzMarkSegments[mark=||, size=2pt](O,P_2);
	\tkzMarkSegments[mark=||, size=2pt](O,P_8);
...
% angles labels %{{{1
	...
	\pic[draw, ->, "$45^\circ$", angle radius=0.8cm, angle eccentricity=1.6]
		{angle = P_8--O--P_1};

	\pic[draw, ->, "$45^\circ$", angle radius=0.8cm, angle eccentricity=1.6]
		{angle = P_1--O--P_2};

	\pic[draw, ->, "$\theta$" {xshift=-12}, angle radius=1.7cm, angle eccentricity=1.1]
		{angle = P_8--O--P_2};
