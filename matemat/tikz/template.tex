...
% coordinates %{{{2
	...
	%\coordinate (D) at ($ (A) + (B) $);
	%\coordinate (D) at ($ (A) + \ratio*(B) $);
	%\coordinate (D) at ($ (A) + \ratio*(v) $);
	%\coordinate (D) at ($ (A) + \ratio*(B) - \ratio*(C) $);
...
% perpendicular coordinates, orthogonal projections %{{{2
	...
	%\path let
		%\p1 = ($(A)!(P)!(B)$)
	%in coordinate (H) at ({max(min(\x1,\xB),\xA)}, {max(min(\y1,\yB),\yA)});
