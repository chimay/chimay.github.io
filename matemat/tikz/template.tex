...
% norm, distance %{{{3
	...
	% ---- 28.45 is a necessary scale factor, unit change

	%\path let \p1 = ($ (B) - (A) $)
	%in
	%\pgfextra{
		%\pgfmathsetmacro{\distAB}{veclen(\x1,\y1)/28.45}
		%\global\let\distAB\distAB
	%};

	%\path let \p1 = ($ (B) - (A) $)
	%in \pgfextra{\pgfmathsetmacro{\dist}{veclen(\x1,\y1)}};
	...
	%in
	%\pgfextra{
		%\pgfmathsetmacro{\dist}{veclen(\x2-\x1,\y2-\y1)}
	%};
...
% perpendicular coordinates, orthogonal projections %{{{2
	...
	% ---- vector AP decomposition in parallel AH and perpendicular HP

	%\coordinate (H) at ($(A)!(P)!(B)$);

	% ---- orthonormal basis

	%\coordinate (E1) at ($(A)!1!(B)$);
	%\coordinate (E2) at ($(A)!1!90:(B)$);

	% ---- reflects P across AB

	%\coordinate (H) at ($(A)!(P)!(B)$);
	%\coordinate (Q) at ($(H)!2!(P)$);

	%\coordinate (Q) at ($(P)!2!($(A)!(P)!(B)$)$);

	% ---- variants

	%\path let \p1 = ($(A)!(P)!(B)$) in
	%coordinate (H) at
	%({max(min(\x1,\xB),\xA)}, {max(min(\y1,\yB),\yA)});

	%coordinate (H) at ($(A)!0.5!($(A)!(P)!(B)$)$)
