...
pour la moyenne des deux angles et :

$$ \delta = \frac{\alpha - \beta}{2} $$

pour la demi-différence. En additionnant ces deux équations, on obtient :
...
La variable $\delta$ représenté l’écart entre $\alpha$ ou $\beta$
et la moyenne $\mu$. Nous disposons donc des relations inverses entre
$\mu,\delta$ et $\alpha,\beta$.
