...
Nous disposons donc des deux relations inverses entre $\alpha$, $\beta$
et $\mu$, $\delta$.
...
La variable $\delta$ représente l’écart entre $\alpha$ ou $\beta$
et la moyenne $\mu$.
...
Comme la somme des angles d’un triangle vaut 180°, on a :
